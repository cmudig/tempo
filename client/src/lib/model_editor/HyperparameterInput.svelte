<script lang="ts">
  ...
  function handleValueChange() {
    try {
      if (type === 'fix') {
        ...
        value = handleUniformValue(
          uniformLower.toString(),
          uniformUpper.toString()
        );
      } else if (type === 'log uniform') {
        value = handleLogUniformValue(
          logUniformLower.toString(),
          logUniformUpper.toString()
        );
      } else if (type === 'grid search') {
  ...
  function handleLogUniformValue(
    lower: string,
    upper: string
  ): [number, number] {
    try {
...
<div class="mt-2 flex w-full gap-2 items-center">
  <div class="text-sm">Value type:</div>
  <select
    bind:value={type}
...
<div class="mt-4 w-full">
  {#if type === 'fix'}
    <div class="text-slate-500 text-xs mb-2">Use this value:</div>
    <div class="flex flex-col gap-2">
      <input
        type="number"
        bind:value={fixedValue}
        placeholder=" Enter fixed value"
        class="flat-text-input w-full {fixedError ? 'border-red-500' : ''}"
  ...
  {:else if type === 'uniform'}
    <div class="text-slate-500 text-xs mb-2 whitespace-pre-line">
      Find the best value between the lower and upper bounds, sampling
      uniformly.
    </div>
      ...
      <div class="flex flex-col gap-2">
        <div class="flex flex-row gap-2 items-center">
          <input
            ...
            class="flat-text-input w-1/2 {uniformError ? 'border-red-500' : ''}"
    ...
    <div class="flex flex-col gap-2">
      <div class="flex flex-row gap-2 items-center">
        <input
          ...
          placeholder=" Lower bound"
          class="flat-text-input w-1/2 {logUniformError
            ? 'border-red-500'
            : ''}"
          on:input={handleValueChange}
        />
  ...
  {:else if type === 'grid search'}
    <div class="text-slate-500 text-xs mb-2">
      Find the best value among the following values (enter numbers separated by
      commas).
    </div>
    <div class="flex flex-col gap-2">
      <input
        ...
        class="flat-text-input w-full {gridError ? 'border-red-500' : ''}"
        on:input={handleValueChange}
      />
      {#if gridError}
