<script>
  ...
  export let valueScale = d3.scaleLinear();

  export let numStops = 100;

  export let numTicks = 7;
  export let logTicks = false;
      ...
      legendAxis = legendAxis
        .tickValues(
          logTicks
            ? d3
                .scaleLog()
                .domain([Math.max(1, valueDomain[0]), valueDomain[1]])
                .ticks(numTicks)
            : linspace(valueDomain[0], valueDomain[1], numTicks)
        )
